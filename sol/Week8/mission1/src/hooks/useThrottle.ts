// useThrottle: 주어진 값(상태)가 변경될 때
// 취소 interval(밀리초) 간격으로만 업데이트해서 성능을 계산한다
...
function useThrottle<T>(value: T, delay: number = 500): T {
  ...
  const lastExcecuted: React.RefObject<number> = useRef<number>(Date.now());
  ...
  useEffect(() => {
    ...
    if (Date.now() >= lastExcecuted.current + delay) {
      ...
      // ClearUp Function 이펙트가 재실행되기 전에 타이머가 실행되지 않았다면
